...
export const ToDoItem: React.FC<Props> = ({todo, toggleTodo, eraseTodo}): JSX.Element => {
    ...
    return(
        <li className="flex justify-between">
            <h1 className={`text-lg font-medium cursor-pointer ${todo.status === "Completed" ? "line-through decoration-red-600 decoration-4" : ""}`} onClick={() => toggleTodo(todo)}> {todo.description} </h1>
